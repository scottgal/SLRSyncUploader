...
namespace NetSync
{
    public class Match
    {
        ...
        private readonly int[] tagTable = new int[TABLESIZE];
        ...
        public void BuildHashTable(SumStruct s)
        {
            ...
            {
                tagTable[(targets[i]).t] = i;
            }
        }

        public void MatchSums(IOStream f, SumStruct s, MapFile buf, int len)
        {
            var fileSum = new byte[CheckSum.MD4_SUM_LENGTH];
        ...
        public void Matched(IOStream f, SumStruct s, MapFile buf, int offset, int i, Sum sum)
        {
            ...
            {
                Log.WriteLine("match at " + offset + " last_match=" + lastMatch + " j=" + i + " len=" + s.sums[i].len +
                              " n=" + n);
            }
        ...
        public void HashSearch(IOStream f, SumStruct s, MapFile buf, int len, Sum _sum)
        {
            ...
            UInt32 k;
            int wantI;
            var sum2 = new byte[CheckSum.SUM_LENGTH];
            UInt32 s1, s2, sum;
            int more;
            byte[] map;
        ...
        public void MatchReport(IOStream f)
        {
            ...
            if (options.amServer)
            {
                f.MultiplexWrite(MsgCode.MSG_INFO, Encoding.ASCII.GetBytes(report), report.Length);
            }
        }
    }
}