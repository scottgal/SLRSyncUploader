...
namespace NetSync
{
    public class Receiver
    {
        ...
        private string LocalizePath(ClientInfo cInfo, string path)
        {
            ...
            {
                ret = path.Substring(path.ToLower().IndexOf(normalized) + normalized.Length).Replace('/',
                                                                                                     Path.
        ...
        public int ReceiveFiles(ClientInfo clientInfo, List<FileStruct> fileList, string localName)
        {
            ...
            while (true)
            {
                ...
                {
                    MainClass.Exit("Invalid file index " + i + " in receiveFiles (count=" + fileList.Count + ")",
                                   clientInfo);
                }
                ...
                {
                    fileName = Path.Combine(options.dir,
                                            LocalizePath(clientInfo, file.GetFullName().Replace(":", String.Empty)).
                                                Replace("\\", "/"));
                    //fileName = Path.Combine(options.dir, file.FNameTo().Replace(":",String.Empty)).Replace("\\", "/");
                    // TODO: path length
                    Directory.CreateDirectory(
                        Path.Combine(options.dir, LocalizePath(clientInfo, file.DirName.Replace(":", String.Empty))).
                            Replace("\\", "/"));
        ...
        public bool ReceiveData(ClientInfo clientInfo, string fileNameR, Stream fdR, long sizeR, string fileName,
                                Stream fd, int totalSize)
        {
            IOStream f = clientInfo.IoStream;
            var fileSum1 = new byte[CheckSum.MD4_SUM_LENGTH];
            var fileSum2 = new byte[CheckSum.MD4_SUM_LENGTH];
            var data = new byte[Match.CHUNK_SIZE];
